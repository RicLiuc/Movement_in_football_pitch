...
#3: center a little bit attacking
#avg_c3
...
def get_frames(dt):
    ...
    c = np.zeros((len(dt['X_p']), 3, 5, 3))
    for i in (range(0, len(dt['X_p']))):
        c[i] = dt['X_c'][i][:3]
...
def plot_changes(steps, point, direction, col_, row_, value, g, team_, ind):
    ...
    'forward-left', 'forward-right', 'behind-left', 'behind-right'
    ...
    plt.title(f"Average changes in each cell after the changes of the players location, Analyzed team, first period frame, Cluster {ind}", fontsize=15)
    plt.show()
...
def plot_changes2(steps, point, direction, col_, row_, value, g, team_, ind):
    ...
    plt.title(f"Average changes in each cell after the changes of the players location, Opponent team, first period frame, Cluster {ind}", fontsize=15)
    plt.show()
...
def change_position(p, team, row_, col_, direction, value):
    ...
    for i in range(0, len(p_mod)):
        if p_mod[i][team][row_][col_] >= value:

            p_mod[i][team][row_][col_] = p_mod[i][team][row_][col_]-value
            p_mod[i][team][row_2][col_2] = p_mod[i][team][row_2][col_2]+value
        else:
            temp=p_mod[i][team][row_][col_]
            p_mod[i][team][row_][col_] = 0
            p_mod[i][team][row_2][col_2] = p_mod[i][team][row_2][col_2] + temp
...
def main_cluster(dt, sum_, meta, centroids):
    ...
    avg_p0, ball_p0 = average_cluster(p0)
    avg_p1, ball_p1 = average_cluster(p1)
    avg_p2, ball_p2 = average_cluster(p2)
    avg_p3, ball_p3 = average_cluster(p3)
    ...
    return clusters_, avg_p0, ball_p0, avg_p1, ball_p1, avg_p2, ball_p2, avg_p3, ball_p3
...
def main_predict(clusters_, ind, team_, row_, col_, direction, value, team):
    ...
    if check_movements(col_, row_, direction):
        ...
        p_mod = change_position(p, team_, row_, col_, direction, value)

        y_pred_mod = predict(c, p_mod, meta, team)
